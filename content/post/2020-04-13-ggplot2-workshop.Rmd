...
ggplot(data = faithful, 
  ...
  geom_point()
# 如果mapping 是放在 ggplot 则所有的 geometry 都公用这个映射
...
这个有点像 phoshop 中的图层概念, geometry 是一层一层往上堆叠的
```{r}
...
```{r}
ggplot(faithful) + 
  geom_point(aes(x = eruptions, y = waiting), size = 3, alpha = 0.5)
```
...
```{r}
ggplot(faithful) + 
  geom_histogram(aes(x = eruptions, fill = waiting <=60), alpha = 0.5)
```
...
ggplot(faithful) + 
  geom_point(aes(x = eruptions, y = waiting)) +
  geom_abline(slope = -10, intercept = 100)
```
...
<font color="darkred"> __Every geom has a stat__ </font>. This is why new data (`count`) can appear when using 
`geom_bar()`., the default `stat` for `geom_bar` is `count`
...
a `stat_*()` with an attached geom. 

大多数人使用 `geom` 时, 使用了默认的 `stat` , 只是不知道而已

Look at `geom_bar()` and figure out which stat it uses as default. Then modify the code to use the stat directly instead
...
mpg %>% 
  count(class, name = "count") %>% 
  ggplot() +
  # 注意这个地方是以及计算了统计量的表格, class and count 已经是 summarise 之后的表格
  geom_bar(aes(x = class, y = count), stat = "identity")
  #geom_col(aes(class, count), stat = "identity")
```
* * *
...
fun 作用于 `aes` 中的 y, x 类似于 `group_by` 中的 factor variable.
```{r}
ggplot(mpg) +
  geom_jitter(aes(x = class, y = hwy), width = 0.2) +
  stat_summary(aes(x = class, y = hwy), fun = "mean", geom = "point", color = "red") +
  stat_summary(aes(x = class, y = hwy), fun = "median", geom = "point", color = "blue")
```
...
#### Exercises
One of the great things about facets is that <font color="darkred"> __they share the axes between the different panels__.</font>  Sometimes this is undiserable though, and <font color="darkred"> __the behaviour can be changed with the `scales` argument__</font> . 
...
> Setting limits on the coordinate system will zoom the plot (like you're looking at it with a <font color="darkred"> __magnifying glass__</font> ), and will not change the underlying data like setting limits on a scale will.
